...
from oxygen.oxyast import OxyCollection, OxyCollectionAccess, OxyDotAccess, OxyRange, OxyVar, OxyVarDecl, OxyAST
from oxygen.grammar import *
from oxygen.utils import error, warning
from oxygen.visitor import (LLVMTypeSymbol, OxyClassSymbol, OxyCollectionSymbol,
                            OxyEnumSymbol, OxyFuncSymbol, OxyNodeVisitor, OxyStructSymbol,
                            OxyTypeSymbol, OxyVarSymbol)
...
def types_compatible(left_type: OxyAST, right_type: OxyAST) -> bool:
    l_type = str(left_type)
    r_type = str(right_type)
    int_type = ('i8', 'i16', 'i32', 'i64', 'int8',
...
class Preprocessor(OxyNodeVisitor):
    def __init__(self, file_name: str):
        super().__init__()
        self.file_name = file_name
        self.return_flag = False

    def check(self, node) -> LLVMTypeSymbol:
        res = self.visit(node)
        if self.unvisited_symbols:
            ...
            if len(sym_list):
                warning('Unused variables ({})'.format(
                    ','.join(sym_name for sym_name in sym_list)))
        return res
    ...
    def visit_switch(self, node):
        ...
        for case in node.cases:
            ...
            if case_type != DEFAULT and case_type is not switch_var.type:
                error('file={} line={}: Types in switch do not match case'.format(
    ...
    def visit_assign(self, node):  # TODO clean up this mess of a function
        ...
        if hasattr(node.right, 'name'):
            if hasattr(node.right, 'name'):
                if isinstance(self.search_scopes(node.right.name), (OxyStructSymbol, OxyEnumSymbol, OxyClassSymbol)):
                    var_name = node.left.value.value
                    value = self.infer_type(node.left.type)
                    value.accessed = True
                    if isinstance(node.right, OxyCollection) or isinstance(node.right, OxyRange):
                        _, collection_type = self.visit(node.right)

                    if value.name in (TUPLE, LIST) and (not isinstance(node.right, OxyRange) and node.right.type != value.name):
                        error('file={} line={}: Contradicting {}-{} declaration'.format(
                            self.file_name, node.line_num, value.name, node.right.type))
                elif hasattr(node.right, 'name') and isinstance(self.search_scopes(node.right.name), (OxyStructSymbol, OxyEnumSymbol, OxyClassSymbol)):
                    var_name = node.left.value
                    value = self.search_scopes(node.right.name)
                    value.accessed = True
                elif isinstance(node.right, OxyCollection) or isinstance(node.right, OxyRange):
                    var_name = node.left.value
                    value, collection_type = self.visit(node.right)
                elif isinstance(node.left, OxyDotAccess):
                    field_assignment = True
                    var_name = self.visit(node.left)
                    value = self.visit(node.right)
                elif isinstance(node.right, OxyDotAccess):
                    var_name = node.left.value
                    value = self.search_scopes(node.right.obj)
                    value.accessed = True
                    value = self.infer_type(value)
                elif isinstance(node.left, OxyCollectionAccess):
        ...
        if not lookup_var:
            if collection_type:
                col_sym = OxyCollectionSymbol(var_name, value, collection_type)
                ...
                if var_name is value:
                    ...
                    error('file={} line={} Type Error: What are you trying to do?!?! (fix this message)'.format(
                        self.file_name, node.line_num))
            elif isinstance(value, OxyFuncSymbol):
                value.name = var_name
                self.define(var_name, value)
            elif hasattr(value, 'name') and value.name == FUNC:
                var = self.visit(node.right)
                if isinstance(var, OxyFuncSymbol):
                    self.define(var_name, var)
                elif isinstance(var, LLVMTypeSymbol):
                    self.define(var_name, var.func)
                else:
                    val_info = self.search_scopes(node.right.value)
                    func_sym = OxyFuncSymbol(var_name, val_info.type.return_type,
                                             val_info.parameters, val_info.body, val_info.parameter_defaults)
                    self.define(var_name, func_sym)
            else:
                var_sym = OxyVarSymbol(var_name, value, node.left.read_only)
                var_sym.val_assigned = True
                self.define(var_name, var_sym)
        else:
            if isinstance(node.left, OxyVarDecl):
                error('file={} line={}: Cannot redefine the type of a declared variable: {}'.format(
                    self.file_name, node.line_num, var_name))

            if collection_assignment:
                col = self.search_scopes(node.left.collection.value)
                if col.type.name == TUPLE:
                    error('file={} line={}: Cannot change the elements of a tuple: {}'.format(
                        self.file_name, node.line_num, var_name))
                elif lookup_var.item_types == value:
                    return
            if lookup_var.read_only:
                error('file={} line={}: Cannot change the value of a variable declared constant: {}'.format(
                    self.file_name, node.line_num, var_name))
            ...
            if hasattr(value, 'type') and lookup_var.type is value.type:
                return
            if isinstance(value, OxyTypeSymbol):
                value.accessed = True
            ...
            if hasattr(value, 'value'):
                if value.value == lookup_var.type.name:
                    return
            error('file={} line={} Type Error: Not good things happening (fix this message)'.format(
                self.file_name, node.line_num))
    ...
    def visit_opassign(self, node):
        ...
        if types_compatible(left_type, right_type) or left_type is any_type or right_type is any_type:
            ...
            error('file={} line={}: Things that should not be happening ARE happening (fix this message)'.format(
                self.file_name, node.line_num))
    ...
    def visit_var(self, node):
        ...
        if val is None:
            error('file={} line={}: Name Error: {}'.format(
                self.file_name, node.line_num, repr(var_name)))
        else:
            if not val.val_assigned:
                error('file={} line={}: {} is being accessed before it was defined'.format(
                    self.file_name, node.line_num, var_name))
            val.accessed = True
            return val
    ...
    def visit_binop(self, node):
        if node.op == CAST or node.op in (IS, IS_NOT):
            ...
            if node.right.value not in TYPES and not isinstance(self.search_scopes(node.right.value), (OxyEnumSymbol, OxyClassSymbol, OxyStructSymbol)):
                error('file={} line={}: type expected for operation {}, got {} : {}'.format(
                    self.file_name, node.line_num, node.op, node.left, node.right))
            ...
            if types_compatible(left_type, right_type) or left_type is any_type or right_type is any_type:
                ...
                error('file={} line={}: types do not match for operation {}, got {} : {}'.format(
                    self.file_name, node.line_num, node.op, left, right))

    def visit_unaryop(self, node):
        return self.visit(node.parse_any_expr)
    ...
    def visit_range(self, node):
        ...
        if left_type in (self.search_scopes(INT), self.search_scopes(DOUBLE), self.search_scopes(FLOAT)) and right_type in (self.search_scopes(INT), self.search_scopes(DOUBLE), self.search_scopes(FLOAT)):
            ...
            error('file={} line={}: Please don\'t do what you just did there ever again. It bad (fix this message)'.format(
                self.file_name, node.line_num))
    ...
    def visit_typedeclaration(self, node):
        typ = OxyTypeSymbol(node.name, self.search_scopes(node.collection.value))
    ...
    def visit_externfuncdecl(self, node):
        ...
        if self.search_scopes(func_name) is not None:
            error('file={} line={}: Cannot redefine a declared function: {}'.format(
                self.file_name, node.line_num, func_name))

        if func_type and func_type.name == FUNC:
            func_type.func = OxyFuncSymbol(ANON, self.visit(
                node.return_type.func_ret_type), node.parameters, node.body, node.parameter_defaults)

        self.define(func_name, OxyFuncSymbol(
            func_name, func_type, node.parameters, None))
        self.new_scope()
        if node.varargs:
            varargs_type = self.search_scopes(LIST)
            varargs_type.type = node.varargs[1].value
            varargs = OxyCollectionSymbol(
        ...
        for k, v in node.parameters.items():
            ...
            if var_type is self.search_scopes(FUNC):
                sym = OxyFuncSymbol(k, v.func_ret_type, None, None)
            elif isinstance(var_type, OxyTypeSymbol):
                var_type.accessed = True
                if var_type.type is self.search_scopes(FUNC):
                    sym = OxyFuncSymbol(k, var_type.type.return_type, None, None)
                else:
                    raise NotImplementedError
            else:
                sym = OxyVarSymbol(k, var_type)
            sym.val_assigned = True
            self.define(sym.name, sym)

        func_symbol = OxyFuncSymbol(func_name, func_type, node.parameters, None)
    ...
    def visit_funcdecl(self, node):
        ...
        self.define(func_name, OxyFuncSymbol(func_name, func_type,
        ...
        for k, v in node.parameters.items():
            var_type = self.search_scopes(v.value)
            if var_type is self.search_scopes(FUNC):
                sym = OxyFuncSymbol(k, v.func_ret_type, v.func_params, None)
            elif isinstance(var_type, OxyTypeSymbol):
                var_type.accessed = True
                if var_type.type is self.search_scopes(FUNC):
                    sym = OxyFuncSymbol(
                        k, var_type.type.return_type, v.func_params, None)
                else:
                    raise NotImplementedError
            else:
                sym = OxyVarSymbol(k, var_type)
        ...
        if self.return_flag:
            ...
            for ret_type in return_types:
                ...
                if infered_type is not func_type and not types_compatible(infered_type, func_type):
                    error('file={} line={}: The actual return type does not match the declared return type: {}'.format(
                        self.file_name, node.line_num, func_name))
        elif func_type is not None:
            error('file={} line={}: No return value was specified for function: {}'.format(
                self.file_name, node.line_num, func_name))
        func_symbol = OxyFuncSymbol(
    ...
    def visit_anonymousfunc(self, node):
        ...
        for k, v in node.parameters.items():
            ...
            sym.val_assigned = True
            self.define(sym.name, sym)
        func_symbol = OxyFuncSymbol(ANON, func_type, node.parameters, node.body)
    ...
    def visit_funccall(self, node):
        ...
        if isinstance(func, (OxyStructSymbol, OxyClassSymbol, OxyEnumSymbol)):
        ...
        for x, param in enumerate(parameters.values()):
            if x < len(node.arguments):
                ...
                if not isinstance(var, OxyFuncSymbol) and (var.type is not None and not types_compatible(var, param_ss) and (param_ss != self.search_scopes(ANY) and param.value != var.name and param.value != var.type.name)):
                    raise TypeError
            else:
                ...
                if func_param_keys[x] not in node.named_arguments.keys() and func_param_keys[x] not in parameter_defaults.keys():
                    error('file={} line={}: Missing arguments to function: {}'.format(
                        self.file_name, node.line_num, repr(func_name)))
                else:
    ...
    # TODO: Finish this, make Symbols for Classes and Methods
    def visit_methodcall(self, node):
        # TODO: hardcoded error for tuple methods, thing of a better way to do it
        if isinstance(self.search_scopes(node.obj), OxyCollectionSymbol) and self.search_scopes(node.obj).type.name == TUPLE:
            if node.name in ('set', 'append'):
                error('file={} line={}: Immutable Error: cannot use `{}` method'.format(
                    self.file_name, node.line_num, node.name))

    def visit_structdeclaration(self, node):
        sym = OxyStructSymbol(node.name, node.fields)
        self.define(sym.name, sym)

    def visit_enumdecl(self, node):
        sym = OxyEnumSymbol(node.name, node.fields)
        self.define(sym.name, sym)
    ...
    def parent_class(self, class_symbol, parent):
        ...
        if parent.base is not None:
            self.parent_class(
                class_symbol, self.search_scopes(parent.base.value))

    def visit_classdeclaration(self, node):
        class_methods = [OxyFuncSymbol(method.name, method.return_type,
                                       method.parameters, method.body) for method in node.methods]
        sym = OxyClassSymbol(node.name, node.base, node.fields, class_methods)
    ...
    def visit_vardecl(self, node):
        ...
        if type_name in (LIST, TUPLE):
            var_symbol = OxyCollectionSymbol(
                var_name, type_symbol, node.type.func_params['0'].value)
            var_symbol.read_only = type_name == TUPLE
        else:
            var_symbol = OxyVarSymbol(var_name, type_symbol)
    ...
    def visit_dotaccess(self, node):
        obj = self.search_scopes(node.obj)
        obj.accessed = True
        if isinstance(obj, OxyEnumSymbol):
            return obj
    ...
    def visit_collectionaccess(self, node):
        ...
        if collection.type is self.search_scopes(LIST) or collection.type is self.search_scopes(TUPLE) or collection.type is self.search_scopes(SET):
            if key is not self.search_scopes(INT) and key.type is not self.search_scopes(INT):
                error('file={} line={}: Something something error... huh? (fix this message)'.format(
                    self.file_name, node.line_num))
            ...
            if key is not self.search_scopes(STR) and key.type is not self.search_scopes(STR):
                error('file={} line={}: Dude....... don\'t (fix this message)'.format(
                    self.file_name, node.line_num))
